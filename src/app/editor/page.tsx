          ...
          <div className={styles.sidebarSection}>
            ...
            <div className={styles.toggleGroup}>
              <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                ...
                <span
                  style={{
                    fontSize: "var(--text-xs)",
                    color: ctx.isUpscaling ? "var(--accent)" : "var(--text-tertiary)",
                  }}
                >
                  {ctx.isUpscaling
                    ? "Upscaling…"
                    : `${ctx.pipelineConfig.upscale.useAI ? "ESRGAN" : "Canvas"} ${ctx.pipelineConfig.upscale.scale}×`}
                </span>
              </div>
            ...
            {ctx.pipelineConfig.upscale.enabled && (
              <div className={styles.toggleGroup} style={{ paddingLeft: "var(--space-2)" }}>
                <label htmlFor="upscale-scale-select" style={{ fontSize: "var(--text-xs)" }}>
                  Scale
                </label>
                <select
                  ...
                  onChange={(e) =>
                    send({
                      ...
                      config: {
                        ...ctx.pipelineConfig,
                        upscale: { ...ctx.pipelineConfig.upscale, scale: Number(e.target.value) },
                      },
                    })
                  }
                  style={{ width: "70px" }}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                ...
                {!ctx.isDetectingText && ctx.detectedRegions.length > 0 && (
                  <span style={{ fontSize: "var(--text-xs)", color: "var(--success)" }}>
                    {ctx.detectedRegions.length} regions removed
                  </span>
                )}
              </div>
            ...
            <div className={styles.toggleGroup}>
              <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                ...
                {ctx.pipelineResult?.optimize && (
                  <span style={{ fontSize: "var(--text-xs)", color: "var(--success)" }}>
                    −{ctx.pipelineResult.optimize.savings}% (
                    {(ctx.pipelineResult.optimize.optimizedSize / 1024).toFixed(1)} KB)
                  </span>
                )}
              </div>
              <label className="toggle" aria-label="Toggle SVG optimization">
                <input
                  ...
                  onChange={(e) => {
                    const updated = {
                      ...ctx.pipelineConfig,
                      optimize: { ...ctx.pipelineConfig.optimize, enabled: e.target.checked },
                    };
          ...
          <div className={styles.sidebarSection}>
            ...
            >
              ...
              <svg
                ...
                strokeWidth="2"
                style={{
                  transform: ctx.showAdvanced ? "rotate(180deg)" : "rotate(0)",
            ...
            {ctx.showAdvanced && (
              <div className={styles.advancedPanel}>
                <div className={styles.selectGroup}>
                  <label className={styles.selectLabel} htmlFor="advanced-mode-select">
                    Mode
                  </label>
                  <select
                    ...
                    onChange={(e) =>
                      updateConfig({ mode: e.target.value as VTracerConfig["mode"] })
                    }
                  >
                ...
                <div className={styles.selectGroup}>
                  <label className={styles.selectLabel} htmlFor="advanced-layering-select">
                    Layering
                  </label>
                  <select
                    ...
                    onChange={(e) =>
                      updateConfig({
                        hierarchical: e.target.value as VTracerConfig["hierarchical"],
                      })
                    }
                  >
                ...
                <SliderParam
                  ...
                  tooltip="Remove clusters smaller than this area (px²)"
                  value={ctx.config.filterSpeckle}
                  min={0}
                  max={100}
                  step={1}
                ...
                <SliderParam
                  ...
                  tooltip="Bits per channel. Higher = more colors (max 7, WASM limit)"
                  value={ctx.config.colorPrecision}
                  min={0}
                  max={7}
                  step={1}
                ...
                <SliderParam
                  ...
                  tooltip="Color difference between layers. Higher = fewer layers"
                  value={ctx.config.layerDifference}
                  min={0}
                  max={255}
                  step={1}
                ...
                <SliderParam
                  ...
                  tooltip="Angle threshold for corner detection (degrees)"
                  value={ctx.config.cornerThreshold}
                  min={0}
                  max={180}
                  step={1}
                  unit="°"
                ...
                <SliderParam
                  ...
                  tooltip="Max segment length before subdivision"
                  value={ctx.config.lengthThreshold}
                  min={1}
                  max={100}
                  step={0.5}
                ...
                <SliderParam
                  ...
                  tooltip="Angle threshold for path splicing (degrees)"
                  value={ctx.config.spliceThreshold}
                  min={0}
                  max={180}
                  step={1}
                  unit="°"
                ...
                <SliderParam
                  ...
                  tooltip="Decimal places in SVG path coordinates"
                  value={ctx.config.pathPrecision}
                  min={0}
                  max={8}
                  step={1}
          ...
          <div className={styles.sidebarSection}>
            ...
            <div className={styles.zoomControls}>
              <button
                ...
                onClick={() => send({ type: "ui.zoom", zoom: Math.max(ctx.zoom * 0.8, 0.1) })}
              >
                −
              </button>
              ...
              <button
                type="button"
                className="btn btn-ghost btn-sm btn-icon"
                onClick={() => send({ type: "ui.zoom", zoom: Math.min(ctx.zoom * 1.2, 10) })}
              >
                +
              </button>
              <button
                type="button"
                className="btn btn-ghost btn-sm"
                onClick={() => send({ type: "ui.resetView" })}
          ...
          {!hasImage ? (
            ...
            >
              <div
                ...
                style={{
                  transform: `translate(calc(-50% + ${ctx.pan.x}px), calc(-50% + ${ctx.pan.y}px)) scale(${ctx.zoom})`,
                }}
              >
